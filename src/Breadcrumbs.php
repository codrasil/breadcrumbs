...
class Breadcrumbs extends Collection implements Contracts\BreadcrumbsInterface
{
    /**
     * The items array.
     *
    ...
    {
        $keys = collect(explode('/', $uri))->reject(function ($uri) {
            return empty($uri) || is_null($uri);
        })->toArray();

        $segments = array_combine($keys, $this->request()->segments());

        return collect($segments)->mapWithKeys(function ($segment, $key) {
            return [
                $key => $this->parameter($this->cleanParam($key))
                     ?? $this->parseText($segment),
            ];
        })->values()->mapWithKeys(function ($segment, $key) {
            return [
                $key => new Crumb([
                    'text' => $segment->{$this->crumbs('field')} ?? $segment ?? null,
                    'url' => implode('/', array_slice($this->request()->segments(), 0, $key + 1)),
                ])
            ];
        });
    }
